...
interface MacOSBleCentralNative {
...
// macOS BLE Central wrapper using Core Bluetooth
export class MacOSBleCentral {
  private nativeModule: MacOSBleCentralNative | null = null;
  private eventListeners: Map<string, Function[]> = new Map();

  constructor() {
    if (Platform.OS === 'macos') {
      try {
        this.nativeModule = NativeModules.MacOSBleCentral;
      } catch (error) {
        console.warn('macOS BLE Central native module not available:', error);
      }
    }
  }

  public isSupported(): boolean {
    return Platform.OS === 'macos' && this.nativeModule !== null;
  }

  public async initialize(): Promise<void> {
    if (!this.isSupported()) {
      throw new Error('macOS BLE Central not supported on this platform');
    }
  ...
  public async startScan(filter?: ScanFilter, options?: ScanOptions): Promise<void> {
    if (!this.isSupported()) {
      throw new Error('macOS BLE Central not supported');
    }
