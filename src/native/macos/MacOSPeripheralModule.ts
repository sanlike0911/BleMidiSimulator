...
// Type definition for the native macOS module
interface MacOSPeripheralNativeModule {
  // Capabilities
...
export class MacOSPeripheralModule implements PlatformPeripheralAdapter {
  private nativeModule: MacOSPeripheralNativeModule;
  private eventEmitter: NativeEventEmitter;
  private eventListeners: Map<PeripheralEvent, Set<(data: PeripheralEventData) => void>> = new Map();

  constructor() {
    if (Platform.OS !== 'macos') {
      throw new Error('MacOSPeripheralModule can only be used on macOS');
    }

    // Get the native module
    const { MacOSBlePeripheral } = NativeModules;
    if (!MacOSBlePeripheral) {
      throw new Error('MacOSBlePeripheral native module not found. Make sure it is properly linked.');
    }

    this.nativeModule = MacOSBlePeripheral as MacOSPeripheralNativeModule;
    this.eventEmitter = new NativeEventEmitter(MacOSBlePeripheral);
  ...
  private parseConnectedClient(data: any): ConnectedClient {
    return {
      ...
      mtu: data.mtu || 185 // macOS typically supports larger MTU
    };
  }
  ...
  public async getCapabilities(): Promise<PeripheralCapabilities> {
    try {
      ...
      console.error('Failed to get macOS peripheral capabilities:', error);
      // Return default macOS capabilities if native module fails
      return {
        canAdvertise: true,
        canRunGattServer: true,
        maxConnections: 8, // macOS supports more connections than Windows
        maxServices: 10,
        maxCharacteristicsPerService: 15,
        supportedAdvertisingModes: ['lowPower', 'balanced', 'lowLatency'],
        supportedTxPowerLevels: ['low', 'medium', 'high'],
      };
    }
  }
  ...
  public async isSupported(): Promise<boolean> {
    try {
      ...
      console.error('Failed to check macOS peripheral support:', error);
      // macOS with Bluetooth LE support should work
      return true;
    }
  }
  ...
  public async startAdvertising(options: AdvertisingOptions): Promise<void> {
    try {
      const macosOptions = {
        ...
        txPowerLevel: options.txPowerLevel || 'medium',
        advertisingMode: options.advertisingMode || 'balanced',
        isConnectable: options.isConnectable !== false,
        timeout: options.timeout
      };

      await this.nativeModule.startAdvertising(macosOptions);
    } catch (error) {
      console.error('Failed to start macOS advertising:', error);
      throw error;
    }
  }
  ...
  public async stopAdvertising(): Promise<void> {
    try {
      ...
      console.error('Failed to stop macOS advertising:', error);
      throw error;
    }
  }
  ...
  public async isAdvertising(): Promise<boolean> {
    try {
      ...
      console.error('Failed to check macOS advertising status:', error);
      return false;
    }
  }
  ...
  public async startGattServer(services: GattService[]): Promise<void> {
    try {
      const macosServices = services.map(service => ({
        uuid: service.uuid,
        type: service.type,
      ...
      await this.nativeModule.startGattServer(macosServices);
    } catch (error) {
      console.error('Failed to start macOS GATT server:', error);
      throw error;
    }
  }
  ...
  public async stopGattServer(): Promise<void> {
    try {
      ...
      console.error('Failed to stop macOS GATT server:', error);
      throw error;
    }
  }
  ...
  public async addService(service: GattService): Promise<void> {
    try {
      ...
      await this.nativeModule.addService(macosService);
    } catch (error) {
      console.error('Failed to add macOS service:', error);
      throw error;
    }
  }
  ...
  public async removeService(serviceUuid: string): Promise<void> {
    try {
      ...
      console.error('Failed to remove macOS service:', error);
      throw error;
    }
  }
  ...
  public async getConnectedClients(): Promise<ConnectedClient[]> {
    try {
      ...
      console.error('Failed to get macOS connected clients:', error);
      return [];
    }
  }
  ...
  public async disconnectClient(clientId: string): Promise<void> {
    try {
      ...
      console.error('Failed to disconnect macOS client:', error);
      throw error;
    }
  }
  ...
  public async sendIndication(clientId: string, serviceUuid: string, characteristicUuid: string, data: ArrayBuffer): Promise<void> {
    try {
      ...
      console.error('Failed to send macOS indication:', error);
      throw error;
    }
  }
  ...
  public async respondToReadRequest(request: CharacteristicRequest, response: CharacteristicResponse): Promise<void> {
    try {
      ...
      console.error('Failed to respond to macOS read request:', error);
      throw error;
    }
  }
  ...
  public async respondToWriteRequest(request: CharacteristicRequest, response: CharacteristicResponse): Promise<void> {
    try {
      ...
      console.error('Failed to respond to macOS write request:', error);
      throw error;
    }
  }
  ...
  public async getState(): Promise<PeripheralState> {
    try {
      ...
      console.error('Failed to get macOS peripheral state:', error);
      throw error;
    }
  }
  ...
  public async destroy(): Promise<void> {
    try {
      ...
      console.error('Error during macOS peripheral cleanup:', error);
      throw error;
    }
  }
}